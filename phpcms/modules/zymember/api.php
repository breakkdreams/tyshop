...
class api{
	...
	*/
	...
	public function public_wechatapp_login()
	{
		...
		//==================	操作失败-验证 START
			//数据不能为空
			if (!$rs['sex'] || !$rs['nickname'] || !$rs['openid'] || !$rs['headimgurl']) {
				$result = [
					'status'=>'error',
					'code'=>-1,
					'message'=>'数据不能为空',
		...
		if($this->_wechat_kaifang==1){	//关闭
			...
			$memberinfo = $this->member_db->get_one(array('wechatapp_openid'=>$rs['openid']));
		}
	...
	/**
	 * 微信小程序_快捷登录
	 * @status [状态] -1数据不能为空/-2请开启微信登录模式,填写配置/-3开启微信开放平台,填写配置/-4帐号已锁定,无法操作/-5微信错误
	 * @param  [type] $sex [*微信性别]
	 * @param  [type] $nickname [*微信昵称]
	 ...
	 * @return [json] [json数组]
	 */
	public function public_wechatxcx_login(){
		$rs = array();
		$rs['sex'] = $_POST['sex'];
		$rs['nickname'] = $_POST['nickname'];
		$rs['unionid'] = $_POST['unionid'];
		...
		//微信密钥
		$this->_wechatxcx_appid = pc_base::load_config('zysystem', 'wechatxcx_appid');	//微信PE appid
		$this->_wechatxcx_appsecret = pc_base::load_config('zysystem', 'wechatxcx_appsecret');	//微信PE appsecret
		$this->_wechat_kaifang = pc_base::load_config('zysystem', 'wechat_kaifang');	//是否开启微信开放平台（0开启、1未开启）
		$this->_wechat_off = pc_base::load_config('zysystem', 'wechat_off');	//是否开启微信登录（0开启、1未开启）
		...
		//==================	操作失败-验证 START
			//数据不能为空
			if (!$rs['sex'] || !$rs['nickname'] || !$rs['code'] || !$rs['headimgurl']) {
				$result = [
					'status'=>'error',
					'code'=>-1,
					'message'=>'数据不能为空',
			...
			$openid_url = 'https://api.weixin.qq.com/sns/jscode2session?appid='.$this->_wechatxcx_appid.'&secret='.$this->_wechatxcx_appsecret.'&js_code='.$rs['code'].'&grant_type=authorization_code';
			$openid = json_decode(_crul_get($openid_url),true);
			//有openid是否有误
			if(isset($openid['errcode'])){
				$result = [
					'status'=>'error',
					'code'=>-5,
					'message'=>$openid['errmsg'],
				];
				exit(json_encode($result,JSON_UNESCAPED_UNICODE|JSON_PRETTY_PRINT));
			}
		...
		$rs['openid'] = $openid['openid'];
		$rs['unionid'] = $openid['unionid'];
		...
		if($this->_wechat_kaifang==1){	//关闭
			...
			$memberinfo = $this->member_db->get_one(array('wechatxcx_openid'=>$rs['openid']));
		}
		...
		if ($memberinfo) {	//登陆
			...
			//==================	操作成功-注册数据 START
				...
				$userinfo['wechat_headimg'] = $rs['headimgurl'];
				$userinfo['wechat_sex'] = $rs['sex'];
				$userinfo['wechatxcx_openid'] = $rs['openid'];
				//记录微信信息
...
//====================================	商品收藏 START
	...
	public function collect_add($id,$userid)
	{
		...
		if($exist){
			...
			//生成商品链接
			$url = APP_PATH."index.php?m=hpshop&c=index&a=goodsinfo&gid=".$id;
			$data = array(
				'pid'=>$cinfo['data']['id'],
				'catid'=>$cinfo['data']['id'],
				'url'=>$url,
